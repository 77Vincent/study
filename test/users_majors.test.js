...
describe('User_Major', () => {
  ...
  it('Create by user = 200', async () => {
    ...
    await request({
      method: 'PUT', url: `${URL}/users_majors`, auth, body: { major_id: [1, 2] },
    })
    await request({
      method: 'PUT', url: `${URL}/users_majors`, auth, body: { major_id: [1, 7] },
    })

    session = await login(USERS[1].mobilephone, PASSWORD)
    auth = { bearer: session.token }
    await request({
      method: 'PUT', url: `${URL}/users_majors`, auth, body: { major_id: [2, 3] },
    })

    session = await login(USERS[2].mobilephone, PASSWORD)
    auth = { bearer: session.token }
    await request({
      method: 'PUT', url: `${URL}/users_majors`, auth, body: { major_id: [3, 4] },
    })

    session = await login(USERS[3].mobilephone, PASSWORD)
    auth = { bearer: session.token }
    await request({
      method: 'PUT', url: `${URL}/users_majors`, auth, body: { major_id: [4, 5] },
    })

    session = await login(USERS[4].mobilephone, PASSWORD)
    auth = { bearer: session.token }
    await request({
      method: 'PUT', url: `${URL}/users_majors`, auth, body: { major_id: [5, 6] },
    })

    session = await login(USERS[5].mobilephone, PASSWORD)
    auth = { bearer: session.token }
    await request({
      method: 'PUT', url: `${URL}/users_majors`, auth, body: { major_id: [6, 7] },
    })
